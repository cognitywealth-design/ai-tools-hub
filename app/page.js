...
export default function AIToolsHub() {
  ...
  const groupedResults = useMemo(() => {
    ...
    filteredTools.forEach(tool => {
      const catName = Object.keys(TOOLS_DATA).find(
        cat => TOOLS_DATA[cat].tools.find(t => t.name === tool.name)
      );
      if (!grouped[catName]) {
        grouped[catName] = [];
      }
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
                        selectedCategory === cat
                          ...
                          : 'bg-slate-800 dark:bg-slate-700 text-slate-300 dark:text-slate-400 hover:bg-slate-700 dark:hover:bg-slate-600'
                      }`}
                    >
                      {cat}
                    </button>
          ...
          {Object.keys(groupedResults).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-center max-w-md">
                Try adjusting your search or filters to find the perfect AI tool for you.
              </p>
            </div>
          ) : (
